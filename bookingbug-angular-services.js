...
var TimeGridSelectAcross = FC.TimeGridSelectAcross = FC.TimeGrid.extend({

    // Renders a visual indication of a selection. Overrides the default, which was to simply render a highlight.
    renderSelection: function renderSelection(span) {
        if (_.isArray(span)) {
            _.each(span, function (each_span) {
                this.renderSelection(each_span);
            }, this);
        } else {
    ...
    computeSelectionSpan: function computeSelectionSpan(span0, span1) {
        ...
        var sortedSpan = { start: dates[0].clone(), end: dates[3].clone() };
        var start = sortedSpan.start;
        var end = sortedSpan.end;
    ...
    renderHighlightSegs: function renderHighlightSegs(segs) {
        ...
        if (this.highlightSegs) {
            this.highlightSegs = this.highlightSegs.concat(segs);
        } else {
...
var AgendaSelectAcrossWeek = FC.AgendaSelectAcrossWeek = AgendaView.extend({

    timeGridClass: TimeGridSelectAcross,
    ...
    reportSelection: function reportSelection(span, ev) {
        if (_.isArray(span)) {
            this.reportSelection({ start: _.first(span).start, end: _.last(span).end });
        } else {
...
FC.views.agendaSelectAcrossWeek = {
    'class': AgendaSelectAcrossWeek,
    duration: { weeks: 1 },
    defaults: {
        allDaySlot: false,
...
(function () {
    ...
    /***
     * @ngdoc directive
     * @name BB.Directives.directive:bbRepeatFrequencyPicker
     * @scope
     * @restrict A
     *
     * @description
     * RepeatFrequency picker that enables user to set repeat frequency by a number of days or weeks
     * The value is stored in ISO8601 format
     */
    angular.module('BB.Directives').directive('bbRepeatFrequencyPicker', bbRepeatFrequencyPickerDirective);

    function bbRepeatFrequencyPickerDirective() {
        return {
            scope: {
                repeats: '=?'
            },
            restrict: 'A',
            require: 'ngModel',
            templateUrl: 'bb_repeat_frequency_picker.html',
            link: function link(scope, element, attrs, ngModel) {

                ngModel.$formatters.push(function (modelValue) {
                    var repeatFrequency = moment.duration(modelValue);
                    if (repeatFrequency.days() % 7 == 0) {
                        return { type: 'Weeks', quantity: repeatFrequency.weeks() };
                    } else {
                        return { type: 'Days', quantity: repeatFrequency.days() };
                    }
                });
                ...
                ngModel.$parsers.push(function (viewValue) {
                    return moment.duration(viewValue.quantity, viewValue.type).toISOString();
                });

                scope.$watch('repeats', function (repeats) {
                    if (repeats == 'weekly') {
                        scope.data = { type: 'Weeks', quantity: 1 };
                    } else if (repeats == 'daily') {
                        scope.data = { type: 'Days', quantity: 1 };
                    }
    ...
    angular.module('schemaForm').config(function (schemaFormProvider, schemaFormDecoratorsProvider, sfPathProvider) {
        ...
        var repeatFrequencyPicker = function repeatFrequencyPicker(name, schema, options) {
            if (schema.type === 'string' && schema.format === 'repeatfrequency') {
                var f = schemaFormProvider.stdFormObj(name, schema, options);
                f.key = options.path;
                f.type = 'repeatfrequency';
                options.lookup[sfPathProvider.stringify(options.path)] = f;
                return f;
            }
        };

        schemaFormProvider.defaults.string.unshift(repeatFrequencyPicker);

        schemaFormDecoratorsProvider.addMapping('bootstrapDecorator', 'repeatfrequency', 'bootstrap_ui_repeatfrequency_form.html');
    });
})();
'use strict';
...
angular.module('BBAdminServices').directive('scheduleEditor', function (uiCalendarConfig, BBModel, ModalForm) {

    var controller = function controller($scope, $attrs, $timeout) {

        $scope.calendarName = 'scheduleEditor';
        ...
        var getEvents = function getEvents(start, end, timezone, callback) {
            ...
            return $scope.schedule.$get('shifts', params).then(function (collection) {
                return collection.$get('shifts', params).then(function (shifts) {
                    shifts = _.map(shifts, function (p) {
                        return new BBModel.Admin.Shift(p);
                    });
                    return callback(shifts);
                });
            });
        };
        ...
        $scope.options = {
            calendar: {
                ...
                select: function select(start, end, jsEvent, view) {
                    var start_date = start.clone();
                    var by_day = [];
                    moment.range(start, end).by('days', function (day) {
                        by_day.push(day.format('dddd'));
                    });
                    start = new Date(Date.parse(start));
                    start = new Date(start.getTime() + 60000 * start.getTimezoneOffset());
                    end = new Date(Date.parse(end));
                    end = new Date(end.getTime() + 60000 * end.getTimezoneOffset());
                    var init_model = {
                    ...
                    ModalForm.new({
                        ...
                        new_rel: 'new_shift_pattern',
                        post_rel: 'shift_patterns',
                        templateUrl: 'edit_shift_pattern_modal_form.html',
                        init_model: init_model,
                        before_submit: function before_submit(form_model) {
                            if (form_model.end_type != 'count') {
                                form_model.repeat_count = null;
                            }
                            form_model.start_time = moment(form_model.start_time).format('HH:mm');
                            form_model.end_time = moment(form_model.end_time).format('HH:mm');
                            return form_model;
                        }
                    });
                ...
                // },
                eventClick: function eventClick(event, jsEvent, view) {
                    console.log(event.$href('shift_pattern'));
                    event.$get('shift_pattern').then(function (response) {
                        console.log('response ', response);
                        var shift_pattern = new BBModel.Admin.ShiftPattern(response);
                        shift_pattern.end_type = 'never';
                        ModalForm.edit({
                            model: shift_pattern,
    ...
    var link = function link(scope, element, attrs) {
        ...
        // scope.getShiftPatternEvents = (start, end) => {
        //     console.log('getShiftPatternEvents');
        //     console.log(scope.schedule);
        //     let params = {
        //         start_date: start.toISOString(),
        //         end_date: end.toISOString(),
        //         single_patterns: true
        //     };
        //     scope.schedule.$get('shift_patterns', params).then((response) => {
        //         return response;
        //     });
        // };
    ...
    return {
        controller: controller,
        link: link,
        templateUrl: 'schedule_editor_main.html',
        // require: 'ngModel',
        scope: {
            options: '=?',
            schedule: '=',
...
angular.module('BBAdminServices').directive('scheduleRuleConfig', function () {

    var controller = function controller($scope, ModalForm) {

        var fetchRuleSets = function fetchRuleSets() {
            $scope.ruleSets = [{
                id: null, name: 'Default'
            }];
            $scope.schedule.$get('rule_sets').then(function (collection) {
                collection.$get('schedule_rule_sets').then(function (response) {
                    $scope.ruleSets = $scope.ruleSets.concat(response);
                });
            });
            $scope.currentRule.model = $scope.ruleSets[0];
        };

        $scope.newRule = function () {
            $scope.schedule.$flush('new_rule_set');
            ModalForm.new({
                base: $scope.schedule,
                title: 'New Rule',
                new_rel: 'new_rule_set',
                post_rel: 'rule_sets',
                success: function success(rule_set) {
                    $scope.ruleSets = $scope.ruleSets.concat(rule_set);
                    $scope.currentRule.model = _.last($scope.ruleSets);
                }
            });
        };

        $scope.editRule = function (rule_set) {
            rule_set.$flush('edit');
            ModalForm.edit({
                model: rule_set,
                title: 'Edit Rule',
                ...
                fail: function fail(response) {
                    $scope.schedule.$flush('rule_sets');
                    fetchRuleSets();
                }
            });
        };

        $scope.deleteRule = function (ruleSet) {
            ruleSet.$del('self').then(function () {
                delete $scope.schedule.rules['R' + ruleSet.id];
                $scope.ruleSets = _.without($scope.ruleSets, ruleSet);
            });
        };
    ...
    return {
        controller: controller,
        templateUrl: 'schedule_rule_config.html',
        scope: {
            options: '=',
            schedule: '=',
            currentRule: '=?',
...
/***
 * @ngdoc service
 * @name BB.Models:AdminScheduleRuleSet
 */ //

angular.module('BB.Models').factory("AdminScheduleRuleSetModel", function ($q, BBModel, BaseModel) {
    return function (_BaseModel) {
        _inherits(Admin_ScheduleRuleSet, _BaseModel);

        function Admin_ScheduleRuleSet(data) {
            _classCallCheck(this, Admin_ScheduleRuleSet);

            return _possibleConstructorReturn(this, _BaseModel.call(this, data));
        }

        return Admin_ScheduleRuleSet;
    }(BaseModel);
});
'use strict';
...
angular.module('BB.Models').factory("AdminShiftModel", function ($q, BBModel, BaseModel) {
    return function (_BaseModel) {
        _inherits(Admin_Shift, _BaseModel);

        function Admin_Shift() {
            _classCallCheck(this, Admin_Shift);
...
angular.module('BB.Models').factory("AdminShiftPatternModel", function ($q, BBModel, BaseModel) {
    return function (_BaseModel) {
        _inherits(Admin_ShiftPattern, _BaseModel);

        function Admin_ShiftPattern(data) {
            _classCallCheck(this, Admin_ShiftPattern);

            if (data.start_time.length == 5) {
                data.start_time = '1970-01-01T' + data.start_time;
            }
            if (data.end_time.length == 5) {
                data.end_time = '1970-01-01T' + data.end_time;
            }
            return _possibleConstructorReturn(this, _BaseModel.call(this, data));
        }

        Admin_ShiftPattern.prototype.$update = function $update(payload) {
            var _this2 = this;

            var defer = $q.defer();
            if (payload.end_type != 'count') {
                payload.repeat_count = null;
            }
            this.$put('self', {}, payload).then(function (response) {
                _this2.constructor(response);
                defer.resolve(_this2);
            }, defer.reject);
            return defer.promise;
        };

        return Admin_ShiftPattern;
    }(BaseModel);
});
'use strict';
